...
class OAuth
{
    ...
    private function authorization()
    {
        if ($this->authbase === null)
            $this->authbase = base64_encode(Zoom::ZOOM_CLIENT_ID . ':' . Zoom::ZOOM_CLIENT_SECRET);
        return $this->authbase;
    }

    private function authHeader($foo='Basic ')
    {
        $bar = $this->authorization();
    ...
    public function getToken()
    {
        if (Validation::validate(['code' => ['require', 33]]))
        {
            ...
            $res = json_decode(httpsCurl($url, $data, $this->authHeader()), true);
            Validation::data($res);
